Feature: Ability to mint while the coins are in cold storage
  ...
  Scenario:
    ...
    And node "Bob" retrieves the public key of the "minting" address
    And node "Cold storage" retrieves the public key of the "cold storage" address
    And node "Bob" generates a new cold minting address "cold minting" with the minting key "minting" and the spending key "cold storage"
    ...
    When node "Alice" finds enough block for a Proof of Stake block to mature
